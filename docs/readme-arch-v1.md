...
> [!NOTE]
> This is a summary. For the full per-service breakdown, formulas, optimization recommendations, and annual projections, see **[cost-arch-v1.md](cost-arch-v1.md)**.

### Architecture-Level Cost Comparison (High Level)

> [!NOTE]
> Per-component figures below are illustrative roll-ups. The Comprehensive Monthly Estimate table further down shows audited totals from [cost-arch-v1.md](cost-arch-v1.md) (corrected 2026-07-03 against AWS ap-southeast-1 public pricing).
...
> [!IMPORTANT]
> The initial EC2 estimate above reflects the always-on line-item baseline only and excludes peak data-transfer costs. The peak column for the Serverless proposal reflects pay-as-you-go (CloudFront data transfer at $0.140/GB in ap-southeast-1). See the [Comparison section in cost-arch-v1.md](cost-arch-v1.md#comparison-with-initial-ec2-proposal) for the apples-to-apples annual comparison including the CloudFront Business flat-rate plan.

### Comprehensive Monthly Estimate (50M Peak Requests, ap-southeast-1)

> [!NOTE]
> Audited 2026-07-03 — prices verified against AWS ap-southeast-1 public pricing where possible. CloudFront + WAF rates verified directly from AWS pricing pages; other per-service rates are knowledge-based and flagged in [cost-arch-v1.md Notes & Disclaimers](cost-arch-v1.md#notes--disclaimers). Added previously-missed services: **Route 53 DNS, Secrets Manager, S3 request ops, WAF vended logs, DynamoDB optional features**.
...
> [!IMPORTANT]
> The optimized scenario assumes **CloudFront plan-switching**: subscribe to the Business plan ($200/mo, no overage charges, bundles CDN + WAF + DNS + logging) for the single election month, then drop to the Free plan ($0/mo) for the 11 idle months. This requires verification with AWS Support — month-to-month switching without penalty is assumed but not confirmed. See [cost-arch-v1.md → Plan-Switching Mechanics](cost-arch-v1.md#plan-switching-mechanics--verify-with-aws).
...
- Detailed formulas, plan-switching mechanics, verification status, and Mermaid cost-visualization charts are in **[cost-arch-v1.md](cost-arch-v1.md)**
...
| IaC | Terraform or CloudFormation/SAM (see [comparison](CLOUDFORMATION.md)) |

---

## Terraform Infrastructure

Infrastructure will be provisioned via **Infrastructure-as-Code**. The current design docs explore two options — **[Terraform](TERRAFORM.md)** (HCL, modular, multi-provider) and **[CloudFormation/SAM](CLOUDFORMATION.md)** (managed state, unified code+infra pipeline). See the comparison below and the linked docs for the full trade-off analysis.
...
| **Dev cost** | ~$29/mo (scaled-down AWS stack with auto-shutdown) + ~$0.11/mo (Terraform state); see **[COSTS-DEV.md](COSTS-DEV.md)** |

**Full details:** [docs/TERRAFORM.md](TERRAFORM.md) — includes 5 UML diagrams (module architecture, OIDC auth flow, CI/CD activity, multi-environment deployment, full lifecycle sequence).

> [!NOTE]
> Interested in CloudFormation/SAM instead? See [docs/CLOUDFORMATION.md](CLOUDFORMATION.md) for a side-by-side comparison, sample template, migration path, and recommendation.
